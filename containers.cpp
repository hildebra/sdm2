...
bool  OutputStreamer::saveForWrite(shared_ptr<DNA> d,int Pair){
	...
	if( d->isPassed()){//DNA is of good qual and should be written out
...
//	}
	...
	report += "Dereplication:\nAccepted " + intwithcommas((int)passed_hits) +" unique sequences ( " 
		+ itos(totCnts) +" counts, " + minCopiesStr;
...
int Filters::findTag(shared_ptr<DNA> d, string&presentBC, int& c_err, 
	...
	int start(-1), stop(-1);
	int idx(-1);
	int scanRegion = 16; //dna region to scan for Tag Seq
...
void Filters::printStats(ostream& give, string file, string outf, bool main) {
	...
	if (!main){
		give << "Rejected:" << intwithcommas((int)(colStats[0].totalRejected - numAccept)) << endl << "Accepted (Mid+High qual): " << intwithcommas((int)numAccept) << " (" << intwithcommas((int)cst.Trimmed) << " were end-trimmed";
	} else {
		if (p2stat) {
...
//		delete tmpDNA;
		...
		if (itCL == seq2CI.end()) {
			...
			cerr << "Found double ID " << segs2 << endl;//
			if (false) {
				cerr << "Aborting.." << endl;
				exit(74);
			}
